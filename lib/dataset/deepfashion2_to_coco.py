...
def make_json(args):
    ...
    target = 'train' if args.split =='train' else 'validation'
    image_dir = os.path.join(args.indir,target,'image')
    sub_index = 0 # the index of ground truth instance
    num_images = 15 if args.debug else len(os.listdir(image_dir)) 
    
    fname = target +'_debug.json' if args.debug else target +'.json'
    output_json =  os.path.join(args.outdir, fname)

    print(' >>> Create Dataset Label for', target)
    print(' >>> Total # of images are', num_images)
    print(' >>> Source: ', image_dir)
    print(' >>> Destination: ', output_json)

    name_desc = tqdm(range(num_images))
    count = 0
    for num in range(1,num_images+1):
        name_desc.update(1)        
        json_name = os.path.join(args.indir, target, 'annos',str(num).zfill(6)+'.json')
        image_name = os.path.join(args.indir, target, 'image',str(num).zfill(6)+'.jpg')
        if (num>=0):
            count+=1
            imag = Image.open(image_name)
            width, height = imag.size
            with open(json_name, 'rb') as f:
                # print(json_name, os.path.exists(json_name))

                temp = json.loads(f.read())
                try:
                    pair_id = temp['pair_id']
                except KeyError as f:
                    print('ERROR', json_name)
...
if __name__ =='__main__':
    ...
    parser.add_argument('--indir', type=str, help='input direcotry path',
        default='/home/peter/workspace/dataset/DeepFashion2' )
    parser.add_argument('--outdir', type=str, help='output direcotry path' ,
        default='/home/peter/workspace/projects/Fashion/intra/data')
    parser.add_argument('--split', type=str, choices=['train','val'], help='select train/val')
